...
def evaluate(args):
    ...
    with open(summary_path, 'w') as f:
        ...
        f.write("For pasting into spreadsheet (Order Overall, her):")
        # use list of keys instead of category_scores.items() to force order to match the spreadsheet
        f.write(str(round(stereo_score / N * 100, 2)) + ", " + ", ".join([str(category_scores[key]['metric']) for key in ['her']]))

    print('=' * 100)
    print("Output written to: " + args.output_file)
    print("summary stats written to: " + summary_path)
    print("For pasting into spreadsheet (Order Overall, her):\n")
    # use list of keys instead of category_scores.items() to force order to match the spreadsheet
    print(str(round(stereo_score / N * 100, 2)) + ", " + ", ".join([str(category_scores[key]['metric']) for key in ['her']]) + "\n")
    print('=' * 100)
